...
$(h, l ,t)$ head, relation, tail의 관계를 Translation, Convolution등의 operation을 이용하여 scoring을 진행. - unsupervised way로 Knowledge graph 학습을 진행하게 된다.
- TransE : entity들의 translation을 통해 relation들을 모델링한다. dissimilarity를 이용.
- ConvE : multi-layer convolutional network를 이용하여 더 표현적인 특징을 학습할 수 있게한다. probability 이영.
...
Definition Encoder의 output을 Knowledge Graph의 head, sense의 candidate를 tail, relation such as *hypernym, part_of* 의로 나온 *loss* 를 이용하여 학습 진행.
...
A3 : EWISE는 rare words에 대해 다른 모델들 보다 높은 F1 스코어를 유지했고, rare senses에 대해 MFS와 LFS를 나누어 비교했을 때 neural baseline보다 MFS를 희생하기는 했지만 더 큰 LFS에서의 F1 스코어를 얻었다.
